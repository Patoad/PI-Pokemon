...
const getDbInfo = async () => {    
    const infoDB = await Pokemon.findAll({
        ...
        attributes: ["id","name","image", "created", "hp", "attack", "defense", "speed", "height", "weight"], 
    });
    return infoDB;    
}
...
router.post('/', async (req, res, next)=>{
    const { name, image, types, height, hp, attack, defense, speed, weight } = req.body;
    try{
        const newPokemon = await Pokemon.create({ name, image, types, height, hp, attack, defense, speed, weight });
